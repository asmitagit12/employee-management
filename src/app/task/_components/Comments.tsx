...
const CommentsPage: React.FC<Props> = ({ taskId }) => {
  ...
  useEffect(() => {
    ...
    fetchItem()
  }, [taskId,user]) // Ensure taskId is part of the dependency array if it changes
  ...
  const onSubmit = async (data: CommentData) => {
    ...
    if (selectedComment && isUpdate) {
      ...
      res = await fetch(`/api/comments/update/${selectedComment.id}`, {
        method: 'PATCH',
      ...
      if (res.ok) {
        ...
        // Update the comment in state with full user info
        setAllComments(prevComments =>
          prevComments.map(comment =>
            comment.id === updatedComment.id
              ? {
                  ...updatedComment, // Use the entire updated comment, including the user info
                  createdBy: {
                    firstName: updatedComment.createdBy.firstName,
                    lastName: updatedComment.createdBy.lastName
                  }
                }
              : comment
          )
      ...
      // Create new comment
      res = await fetch('/api/comments/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      })
      ...
      if (res.ok) {
        const newComment = await res.json() // Ensure the new comment includes the sender's info
        toast({
          title: 'Success',
        ...
        setAllComments(prevComments => [
          {
            ...newComment.comment, // Get the comment object directly
            createdBy: {
              firstName: newComment.comment.createdBy.firstName,
              lastName: newComment.comment.createdBy.lastName
            }
          },
          ...prevComments
        ])
