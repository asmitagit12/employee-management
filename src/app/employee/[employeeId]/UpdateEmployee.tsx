...
const UpdateEmployee = () => {
    ...
    if (params) {
      const fetchItem = async () => {
        const response = await fetch(`/api/user/id`, {
          method: 'POST',
          body: JSON.stringify({ id: params.employeeId })
        })
  ...
  const onSubmit = async (data: formType) => {
    ...
    const res = await fetch(`/api/employee/updateuser/${params?.employeeId}`, { // userId should be dynamically passed
      method: 'PUT',
    ...
    if (res.ok) {
      ...
      toast({
        title: 'Failed',
        description: errorData?.message || 'Error updating employee',
        variant: 'destructive'
      });
    }
  };
